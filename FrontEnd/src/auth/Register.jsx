...
function Register() {
  ...
  const handleSubmit = (event) => {
    ...
    axios
      .post("https://backend.pedulisehatmu.my.id/register", values)
      .then((res) => {
        if (res.data.Status === "Success") {
          navigate("/login");
        } else {
          // Handle specific error cases
          if (res.data.Error === "Error data di Server") {
            setErrors((prev) => ({ ...prev, email: "Email sudah terpakai" }));
          } else if (res.data.Error === "Format email tidak valid") {
            setErrors((prev) => ({
              ...prev,
              email: "Format email tidak valid",
            }));
          } else if (res.data.Error === "Password minimal 8 karakter") {
            setErrors((prev) => ({
              ...prev,
            ...
            setErrors((prev) => ({
              ...prev,
              email: res.data.Error || "Terjadi kesalahan pada server",
            }));
          }
        }
      })
      .catch((err) => {
        if (err.response) {
          ...
          if (err.response.data && err.response.data.Error) {
            ...
            setErrors((prev) => ({
              ...prev,
              email: "Gagal terhubung ke server",
            }));
          }
        } else {
          setErrors((prev) => ({
